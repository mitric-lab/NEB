        ...
        for self.istep in range(0, nsteps):
            ...
            for i in range(0, len(self.R)):
                ...
                if self.istep == 0:
                    ...
                    Rnext[i] = (2.0-friction)*self.R[i] - (1.0-friction)*Rlast[i] + old_div(self.effF[i],self.mass*pow(dt,2))
                    Rlast[i] = self.R[i]
                    self.R[i] = Rnext[i]
    ...
    def splineMEProfile(self):
        ...
        for i in range(0,n):
            if i == 0:
                ...
                x[i] = x[i-1] + norm(t)
            f[i] = self.V[i]
            f1[i] = dot(self.F[i], old_div(t,norm(t)))
        def MEfunc(rxc):
            """
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--optimize_endpoints", dest="optimize_endpoints", action="store_true", default="False",
    ...
    ****************************
    ...
    # save initial path
    neb.plot()
    neb.findMEP(tolerance=args.tolerance, nsteps=args.nsteps,
