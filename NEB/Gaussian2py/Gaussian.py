        ...
        if "Forces (Hartrees/Bohr)" in l:
            ...
            while True:
                ...
                if "--" in l:
                    ...
                    forces[int(center)-1] = (int(atnum), np.array(list(map(float, [Fx,Fy,Fz]))))
            break
    if len(forces) == 0:
...
def read_symmetric_matrix(fh, N):
    ...
    def read_block(fh, jbl):
        l = fh.readline()
        cols = list(map(int, l.strip().split()))
        for i in range(jbl,N):
            l = fh.readline().replace("D", "E")
            parts = l.strip().split()
            row = int(parts[0])
            mijs = list(map(float, parts[1:]))
...
def write_input(com_file, atomlist, chk_file="", route="", title="",
    ...
    for i,(Zi,posi) in enumerate(atomlist):
        x,y,z = [pos*bohr_to_angs for pos in posi]
