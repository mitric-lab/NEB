    ...
    for b in range(0, nblocks):
        ...
        for row in range(0,len(row_labels)):
            if row_labels[row] == "-":
                ...
                for col in range(b*block_len, min(n,(b+1)*block_len)):
                    s = format % M[row-nr_sep,col]
                    if len(s) > colwidth:
                        s = "%.2e" % M[row-nr_sep,col]
                    txt += string.center(s, colwidth+1) + "|"
            txt += "\n"
        txt += "\n"
    return txt
...
def call_with_opts_from_dict(func):
    ...
    """
    argnames = func.__code__.co_varnames[hasattr(func, 'im_func'):func.__code__.co_argcount]
    ...
    def _f(*args, **opts):
        try:
            ...
            for io,optname in enumerate(optnames):
                if optname in list(opts.keys()): 
                    actualopts[optname] = opts[optname]
                else:
                    actualopts[optname] = func.__defaults__[io]
#            print( "Calling %s with arguments = %s and options = %s" % (func.func_name, actualargs, actualopts) )
        except TypeError:
            raise TypeError('%s(...) requires arg(s) %r'%(
            func.__name__, [argname for argname in argnames]))
