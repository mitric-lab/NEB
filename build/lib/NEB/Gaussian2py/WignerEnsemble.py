...
def center_of_mass(pos, masses):
    ...
    com = old_div(np.array([np.sum(xm[0::3]), np.sum(xm[1::3]), np.sum(xm[2::3])]),M)
    return com
...
def tensor_of_inertia(pos, masses):
    ...
    """
    I = np.zeros((3,3))
    for i in range(0, old_div(len(pos),3)):
...
def vibrational_analysis(hess, masses, zero_threshold=1.0e-9, is_molecule=False):
    ...
    hess_mwc = old_div(hess, np.sqrt(np.outer(masses, masses)))
...
def wigner_distribution(x0, hess, masses, zero_threshold=1.0e-9, is_molecule=False):
    ...
    Ap += old_div(np.dot(Li, np.dot(Oconstr, Li.transpose())) * MsqInv * Msq.max(),MsqInv.max())
